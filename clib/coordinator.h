...
private:
    ...
    uint32_t rn= 10; //随机数（正数）
    ASS ssrn_s; //coordinator与客户之间秘密分享的随机数的份额，用于验证时掩盖上32位
...
public:
    ...
    uint64_t midres_ep[2] = {0,0}; //接受客户返回的开发阶段的中间结算结果的线性组合
    ...
    void KeyGen(uint32_t mackey_value){
       //初始化MAC key
    ...
    uint32_t GenRandom(){
        uint64_t rn_s = (uint64_t)rn*fixed;
        // rn = GenRand32();
        // int32_t rn_s = rn;
        ssrn_s.value_share.share= rn_s/2; //随机数的可验证秘密分享，用于验证时掩盖上32位
        ssrn_s.tag_share.share=(uint64_t)(rn_s*bate.key1/2); //随机数的可验证秘密分享，用于验证时掩盖上32位
        ...
        for(j=0; j<num_clients; j++){
            ...
            for(i=0; i<K; i++)
                if(j==E[i]){
                    ...
                    sssys_u[E[i]][0].value_share.share = (uint64_t)(sys_u_ui+rn_s);
                    sssys_u[E[i]][0].tag_share.share = (uint64_t)((bate.key1 * sys_u_ui)+rn_s);
                    sssys_u[E[i]][1].value_share.share = (uint64_t)(-rn_s);
                    sssys_u[E[i]][1].tag_share.share = (uint64_t)(-rn_s);
                    //cout<<sys_u[E[i]]<<endl;
                }
        }
    ...
    void Getsstag_sat_u(paillier_ciphertext_t * ct, int index){ //在更新阶段获取客户的统计效用的标签的份额
        ...
        sstag_sat_u[index].share = (uint64_t)(*(pt.m->_mp_d));
    ...
    void Updatesstag_sta_u_tu(int r, int l, int index){
        ...
        sstag_sat_u[index].share = sstag_sat_u[index].share + (int64_t)(bate.key1 * ((int64_t)(tu * (uint32_t)(1<<(fixed_point-1)))));
    ...
    void GetMidres_EP(uint64_t midres1, uint64_t midres2){
        midres_ep[0] = midres1;
        midres_ep[1] = midres2;
    }
    ...
    bool SingleCheck_S(string signal, ASS ss0, ASS ss1, ASS ss2, double * res){
        ...
        if(signal=="check_three_shares"){
            a = (int32_t)(ss0.value_share.share+ss1.value_share.share+ss2.value_share.share);//恢复出秘密值
            *res = (double)a/fixed;
            //检查正确性
            tag = (int64_t)(ss0.tag_share.share+ss1.tag_share.share+ss2.tag_share.share);
            value = (int64_t)(ss0.value_share.share+ss1.value_share.share+ss2.value_share.share-(ss0.constant+ss1.constant+ss2.constant)/3);
            //cout<<tag<<","<<value<<","<<(float)(abs(tag-bate.key1*value))/(uint32_t)(1<<(fixed_point-1))<<endl;
        }
        else if(signal=="check_two_shares"){
            //cout<<ss1.value_share.share<<","<<ss2.value_share.share<<endl;
            //cout<<"value with masked upper k bits"<<(uint64_t)((ss1.value_share.share+ss2.value_share.share))<<endl;//经检查，确实带有掩盖
            a = (int32_t)(ss0.value_share.share+ss1.value_share.share);//恢复出秘密值
            *res = (double)a/fixed;
            //检查正确性
            tag = (int64_t)(ss0.tag_share.share+ss1.tag_share.share);
            value = (int64_t)(ss0.value_share.share+ss1.value_share.share-ss0.constant);
            //cout<<tag<<","<<value<<","<<(float)(abs(tag-bate.key1*value))/(uint32_t)(1<<(fixed_point-1))<<endl;
        }
