...
private:
    ...
    __uint128_t EPcheck=0; //用于开发阶段计算结果的验证
public:
    ...
    // Client(double loss){
    //     this->sta_u=(int128_t)(loss * (uint128_t)(1<<(fixed_point-1)));
    // }
    ...
    void Secret_sharing_C(paillier_ciphertext_t * ct, paillier_pubkey_t * pk){
        ...
        __uint128_t rn_c = (__uint128_t)rn*fixed;
    ...
    SS Reshare_value(void){ //将自己的统计效用的秘密份额分享出去
        ...
        __int128_t rn_c = (__int128_t)rn*fixed;
        SS sssta_u1;
        sssta_u0.value_share.share = -rn_c;
        sssta_u0.tag_share.share = -rn_c;
        sssta_u1.share = (__int128_t)(sta_u+rn_c);
        // cout<<"C.reshare_v";
    ...
    void Reshare_tag(__int128_t rn_s){
    ...
    void recordMidresult(__uint128_t midres){
        EPcheck+=midres;
    }

    __uint128_t revealMidresult(){
        return EPcheck;
    }
    ...
    ASS SecureConstMul(__uint128_t c, ASS ss){
        clock_t start,end;
        start=clock();

        ASS res;
        res.value_share.share = (__uint128_t)((c*ss.value_share.share));
        res.tag_share.share = (__uint128_t)((c*ss.tag_share.share));
        res.constant = (__uint128_t)(c*ss.constant);
    ...
    void SecureMul(string signal, __uint128_t e, __uint128_t f){//这里容易有上下溢出，需要进行截断，利用SecureML中的截断协议，对每个份额进行一次截断
        ...
        if(signal=="mul_of_self_u"){//开发阶段计算客户自己的效用值
            ...
            // ss_u0.value_share.share=(uint128_t)((e*f+e*ssMT.bshare.value_share.share+f*ssMT.ashare.value_share.share+ssMT.cshare.value_share.share)/fixed);
            // ss_u0.tag_share.share=      (uint128_t)((e*ssMT.bshare.tag_share.share+f*ssMT.ashare.tag_share.share+ssMT.cshare.tag_share.share)/fixed); 

            ss_u0.value_share.share=(__uint128_t)((e*f+eb.value_share.share+fa.value_share.share+ssMT.cshare.value_share.share)/fixed); 
            ss_u0.tag_share.share=      (__uint128_t)((eb.tag_share.share+fa.tag_share.share+ssMT.cshare.tag_share.share)/fixed);
            ss_u0.constant = (__uint128_t)((e*f+eb.constant+fa.constant+ssMT.cshare.constant)/fixed);
            //cout<<"ssu0: "<<ss_u0.value_share.share<<endl;
        }
        else if(signal=="mul_of_other_u"){//开发阶段计算对方客户的效用值
            ASS eb=SecureConstMul(e, ssMT.bshare);
            ASS fa=SecureConstMul(f, ssMT.ashare);
            ss_u1.value_share.share=(__uint128_t)((eb.value_share.share+fa.value_share.share+ssMT.cshare.value_share.share)/fixed);
            ss_u1.tag_share.share=  (__uint128_t)((eb.tag_share.share+fa.tag_share.share+ssMT.cshare.tag_share.share)/fixed);
            ss_u1.constant = (__uint128_t)((e*f+eb.constant+fa.constant+ssMT.cshare.constant)/fixed);
            //cout<<"ssu1: "<<ss_u1.value_share.share<<endl;
        }
        else if(signal=="mul_of_lc_in_c1"){//用于比较双方效用值大小
            ASS eb=SecureConstMul(e, ssMT.bshare);
            ASS fa=SecureConstMul(f, ssMT.ashare);
            sslc.value_share.share=(__uint128_t)((e*f+eb.value_share.share+fa.value_share.share+ssMT.cshare.value_share.share)/fixed);
            sslc.tag_share.share=      (__uint128_t)((eb.tag_share.share+fa.tag_share.share+ssMT.cshare.tag_share.share)/fixed);
            sslc.constant = (__uint128_t)((e*f+eb.constant+fa.constant+ssMT.cshare.constant)/fixed);
        }
        else if(signal=="mul_of_lc_in_c2"){//用于比较双方效用值大小
            ASS eb=SecureConstMul(e, ssMT.bshare);
            ASS fa=SecureConstMul(f, ssMT.ashare);
            sslc.value_share.share=(__uint128_t)((eb.value_share.share+fa.value_share.share+ssMT.cshare.value_share.share)/fixed);
            sslc.tag_share.share=  (__uint128_t)((eb.tag_share.share+fa.tag_share.share+ssMT.cshare.tag_share.share)/fixed);
            sslc.constant = (__uint128_t)((e*f+eb.constant+fa.constant+ssMT.cshare.constant)/fixed);
        }
        else
