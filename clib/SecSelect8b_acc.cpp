                                  ...
                                  int E[], const double sta_u[], const double D[], const int L[], double ssu[], int rounds)
{
    ...
    int16_t rn_s = SC.GenRandom(); //rn_s
...
bool SecComp(Coordinator * SC, Client * C, int c1, int c2){ //在SC的协助下进行安全比较
    ...
    //重分享客户的统计效用
    int16_t rn_s=(int16_t)SC->SendRandom();
    ...
    uint16_t e_u[2] = {sse_u[0].value_share.share+sse_u[1].value_share.share, sse_u[2].value_share.share+sse_u[3].value_share.share};
    uint16_t f_u[2] = {ssf_u[0].value_share.share+ssf_u[1].value_share.share, ssf_u[2].value_share.share+ssf_u[3].value_share.share};
    ...
    C[c1].SecureMul("mul_of_self_u", (uint8_t)e_u[0], (uint8_t)f_u[0]); C[c2].SecureMul("mul_of_self_u", (uint8_t)e_u[1], (uint8_t)f_u[1]);
    C[c1].SecureMul("mul_of_other_u", (uint8_t)e_u[1], (uint8_t)f_u[1]); C[c2].SecureMul("mul_of_other_u", (uint8_t)e_u[0], (uint8_t)f_u[0]);
    ...
    uint16_t e_lc = sse_lc[0].value_share.share+sse_lc[1].value_share.share;
    uint16_t f_lc = ssf_lc[0].value_share.share+ssf_lc[1].value_share.share;
    C[c1].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    C[c2].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    //cout<<"e_lc: "<<(double)(int8_t)(uint8_t)e_lc/fixed; cout<<"; f_lc: "<<(double)(int8_t)(uint8_t)f_lc/fixed<<endl;; //以上四行正确
    C[c1].SecureMul("mul_of_lc_in_c1", (uint8_t)e_lc, (uint8_t)f_lc); C[c2].SecureMul("mul_of_lc_in_c2", (uint8_t)e_lc, (uint8_t)f_lc);
    //C[c1].showSS(6); C[c2].showSS(6); // 以上六行正确
    //打开比较结果lc，通过lc的符号来判断两个效用值的大小，lc<0 or lc>0。
    uint16_t lc_masked = C[c1].SingleCheck_C("reveal_lc").value_share.share+C[c2].SingleCheck_C("reveal_lc").value_share.share;
    uint8_t lc_int = (uint8_t)(lc_masked);
    double lc = (double)(int8_t)lc_int/fixed;
...
int main(){
    ...
    double sta_u[6]={1, 3, 2, 7, 5, 6};//sta_u[5]={1.3, 3.5, 2.5, 2.54, 5.7};sta_u[6]={1.3, 3.5, 2.5, 2.54, 5.7, 1.2};
    int T = 10;
    ...
    for(int i=0;i<SC.K;i++){
        //cout<<E[i]<<": ";
        C[E[i]].GetstaU_TU(2, L[E[i]]);
        SC.Updatesstag_sta_u_tu(2, L[E[i]], E[i]);
    }
    ...
    clock_t start,end;
    start=clock();
    SC.Mergesort(non_E, 0, 20-SC.K-1);
    end=clock();

    /**筛选阶段**/
    SC.Select_Client(20, non_E, E); //加入这个函数后就导致执行内存溢出，删掉依概率选取客户后不会产生溢出
