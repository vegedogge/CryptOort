...
int test(){
    ...
    int E[6]={0,1,2,3,4,5};//E[5]={2,4,5,6,8};E[3]={2,4,5}; //如果有元素是10，则会报错段错误
    int sortedE[6];
    long double sta_u[6]={1.3, 3.5, 2.5, 2.54, 5.7, 1.2};//sta_u[5]={1.3, 3.5, 2.5, 2.54, 5.7};sta_u[6]={1, 3, 2, 7, 5, 6};
    for(int i=0; i<6; i++)  {sta_u[i]=dis(gen);E[i]=i;sortedE[i]=i;}
    int T = 10;
    ...
    double ssu[100];
    Coordinator SC(30, 6, 0.8, 20, 1);
    Client C[20];
    uint32_t mac_key_value=2;

    //for(int i=0; i<nselect; i++)    E[i]=E_c[i];

    /*初始化调度员SC与客户*/
    SC.KeyGen(mac_key_value);
    int64_t rn_s = SC.GenRandom(); //rn_s
    // cout<<"SC.GenRandom success! rn="<<rn_s<<endl;
    // cout<<"SC.KeyGen success!"<<endl;
    int non_E[20-SC.K]; //未被开发的客户的编号
    int k = 0;
    for(int i=0; i<20; i++){//划分客户集合为上轮参与训练的被开发的客户与未被开发的客户
        ...
        for(int j=0; j<SC.K; j++){
            if(i == E[j]){
                ...
                C[i].GetstaU(sta_u[j]); //将浮点类型的统计效用转换为定点数，小数位为16
    ...
    for (int i = 0; i < 5; ++i) {
        int minIndex = i;
        for (int j = i + 1; j < 6; ++j) {
            if (sta_u[sortedE[j]] < sta_u[sortedE[minIndex]]) {
                minIndex = j;
            }
        }
        // 交换元素
        double temp = sortedE[i];
        sortedE[i] = sortedE[minIndex];
        sortedE[minIndex] = temp;
    }
    ...
    for (int i = 0; i < 6; ++i) {
        if(sortedE[i]!=E[i]){
            res=0;
            cout<<"stau is diff from soartedstau"<<endl;
        }
    }
    return res;
}
...
bool SecComp(Coordinator * SC, Client * C, int c1, int c2){ //在SC的协助下进行安全比较
    ...
    //重分享客户的统计效用
    int64_t rn_s=(int64_t)SC->SendRandom();
    ...
    uint64_t e_u[2] = {sse_u[0].value_share.share+sse_u[1].value_share.share, sse_u[2].value_share.share+sse_u[3].value_share.share};
    uint64_t f_u[2] = {ssf_u[0].value_share.share+ssf_u[1].value_share.share, ssf_u[2].value_share.share+ssf_u[3].value_share.share};
    ...
    C[c1].SecureMul("mul_of_self_u", (uint32_t)e_u[0], (uint32_t)f_u[0]); C[c2].SecureMul("mul_of_self_u", (uint32_t)e_u[1], (uint32_t)f_u[1]);
    C[c1].SecureMul("mul_of_other_u", (uint32_t)e_u[1], (uint32_t)f_u[1]); C[c2].SecureMul("mul_of_other_u", (uint32_t)e_u[0], (uint32_t)f_u[0]);
    ...
    uint64_t e_lc = sse_lc[0].value_share.share+sse_lc[1].value_share.share;
    uint64_t f_lc = ssf_lc[0].value_share.share+ssf_lc[1].value_share.share;
    C[c1].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    C[c2].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    //cout<<"e_lc: "<<(double)(int32_t)(uint32_t)e_lc/fixed; cout<<"; f_lc: "<<(double)(int32_t)(uint32_t)f_lc/fixed<<endl;; //以上四行正确
    C[c1].SecureMul("mul_of_lc_in_c1", (uint32_t)e_lc, (uint32_t)f_lc); C[c2].SecureMul("mul_of_lc_in_c2", (uint32_t)e_lc, (uint32_t)f_lc);
    //C[c1].showSS(6); C[c2].showSS(6); // 以上六行正确
    //打开比较结果lc，通过lc的符号来判断两个效用值的大小，lc<0 or lc>0。
    uint64_t lc_masked = C[c1].SingleCheck_C("reveal_lc").value_share.share+C[c2].SingleCheck_C("reveal_lc").value_share.share;
    uint32_t lc_int = (uint32_t)(lc_masked);
    double lc = (double)(int32_t)lc_int/fixed;
...
void SecMerge(Coordinator * SC, Client * C, int* P, int low, int mid, int hight)  //合并函数
{
	...
	while (i <= mid && j <= hight)
	{
        // cout<<"Compare clients: "<<P[i]<<", "<<P[j]<<" "<<endl;
