...
#include "coordinator64.h"
#include "client64.h"
#include "fixed_random64.h"
                                  ...
                                  int E[], const double sta_u[], const double D[], const int L[], double ssu[], int rounds)
{
    ...
    __int128_t rn_s = SC.GenRandom(); //rn_s
...
bool SecComp(Coordinator * SC, Client * C, int c1, int c2){ //在SC的协助下进行安全比较
    ...
    //重分享客户的统计效用
    __int128_t rn_s=(__int128_t)SC->SendRandom();
    ...
    __uint128_t e_u[2] = {sse_u[0].value_share.share+sse_u[1].value_share.share, sse_u[2].value_share.share+sse_u[3].value_share.share};
    __uint128_t f_u[2] = {ssf_u[0].value_share.share+ssf_u[1].value_share.share, ssf_u[2].value_share.share+ssf_u[3].value_share.share};
    ...
    C[c1].SecureMul("mul_of_self_u", (uint64_t)e_u[0], (uint64_t)f_u[0]); C[c2].SecureMul("mul_of_self_u", (uint64_t)e_u[1], (uint64_t)f_u[1]);
    C[c1].SecureMul("mul_of_other_u", (uint64_t)e_u[1], (uint64_t)f_u[1]); C[c2].SecureMul("mul_of_other_u", (uint64_t)e_u[0], (uint64_t)f_u[0]);
    ...
    __uint128_t e_lc = sse_lc[0].value_share.share+sse_lc[1].value_share.share;
    __uint128_t f_lc = ssf_lc[0].value_share.share+ssf_lc[1].value_share.share;
    C[c1].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    C[c2].recordMidresult(e_lc+f_lc); //客户计算并保存计算中间结果的线性组合
    //cout<<"e_lc: "<<(double)(int64_t)(uint64_t)e_lc/fixed; cout<<"; f_lc: "<<(double)(int64_t)(uint64_t)f_lc/fixed<<endl;; //以上四行正确
    C[c1].SecureMul("mul_of_lc_in_c1", (uint64_t)e_lc, (uint64_t)f_lc); C[c2].SecureMul("mul_of_lc_in_c2", (uint64_t)e_lc, (uint64_t)f_lc);
    //C[c1].showSS(6); C[c2].showSS(6); // 以上六行正确
    //打开比较结果lc，通过lc的符号来判断两个效用值的大小，lc<0 or lc>0。
    __uint128_t lc_masked = C[c1].SingleCheck_C("reveal_lc").value_share.share+C[c2].SingleCheck_C("reveal_lc").value_share.share;
    uint64_t lc_int = (uint64_t)(lc_masked);
    double lc = (double)(int64_t)lc_int/fixed;
